...
- **동적 UI 안내 & 통합 Save/Share 함수**: `ItinerarySidebar.tsx`에 제목 변경 시 `"💡 제목이 변경되어 새 일정으로 저장됩니다."` 안내 뱃지 추가 및 버튼 문구 자동 전환(`새 일정 저장`, `새 이름으로 저장`, `내 일정으로 저장`). 저장과 공유 버튼 모두 공통 `executeSaveOrShare` 함수를 사용하도록 통일.
...
## 이슈 34: 기본 일정 저장 버튼 표기 문구 "저장"으로 원복

### 1. 지적 및 문제점
- **지적 내용**: 저장 버튼의 기본 fallback 텍스트가 "변경사항 저장"으로 변경되었던 것에 대한 직관성 원복 요청.

### 2. 해결 대책 (Fix)
- `ItinerarySidebar.tsx` 내 `saveButtonLabel` 연산의 기본 fallback 문구를 `"변경사항 저장"`에서 **`"저장"`**으로 수정 완료. (제목 변경 시 `"새 이름으로 저장"`, 공유 원본 진입 시 `"내 일정으로 저장"` 표기는 동적 안내용으로 유지).
